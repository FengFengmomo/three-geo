...
class MapHelper {
    constructor(options={}) {
        ...
        if (_enableTiles) { // ?? when cache off, often getting 504 errors per CORS ........
            ...
            L.tileLayer('https://webst01.is.autonavi.com/appmaptile?x={x}&y={y}&z={z}&style=6', {
                foo: 'bar',
    ...
    static mkBuildMarker(ll, onBuild, onCancel) {
        ...
        aBuild.appendChild(document.createTextNode('Build Terrain')); //设置新贴图的链接
        aBuild.href = '#';
        ...
        aCancel.appendChild(document.createTextNode('[取消]')); // 取消按钮
        aCancel.href = '#';
    ...
    static mkBboxLayers(origin, ftBbox) {
        ...
        let line = turfHelpers.lineString([llOrig, llSw]); // 中心点和西南角的点连线，颜色为白色
    ...
    plotOrbit(orbitData) {
        ...
        // add orbitCircle 右键点击时在地图上绘制红色的圈
